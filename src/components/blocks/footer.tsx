...
export const FooterBlock = Config.type("🦶 Footer", {
  fields: {
    ...BasicBlock,
...
function getLinkColumns(block: FooterBlock) {
  ...
  for (let i = 0; i < linkListColumns; i++) {
    linkListColumnsArray.push(
      linkList.slice(i * linkListColumnLength, (i + 1) * linkListColumnLength),
    );
  }
      ...
      <div className="flex flex-wrap items-start justify-between gap-y-32">
        ...
        <div className="flex flex-wrap gap-x-32 gap-y-8">
          {linkColumns.map((linkColumn, index) => {
            return (
              <ul
                key={`footer-link-list-${index}`}
                className="list-none m-0 p-0 ds-stack-8"
              >
                {linkColumn.map((link, index) => {
                  return (
                    <li
                      className="leading-snug"
                      key={`footer-link-list-cell-${index}`}
                    >
                      <a
                        href={link.url}
