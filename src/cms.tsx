...
export async function getFlowPageChildren(path: string[]) {
  ...
  return cms.find(Query.whereParent(parentId).select({
    page: FormPageSchema
  }));
}
...
export async function getPage(schema: CmsPageSchema, path: string[]) {
  ...
  const pages = (await cms.find(Query(schema).where({slug: entrySlug}).select({
    page: schema,  
    parent: Query.parent,
    entryId: Query.id,
    path: Query.path
...
export async function getPageParent(page: string) {  
  ...
  return cms.find(Query.whereId(parentEntry?.parent ?? "")
  .select({
    page: PageSchema,
    })
  .first()
